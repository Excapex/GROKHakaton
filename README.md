...
- Nalaznik **ne izdaje saglasnost**. Prihvatanje izmene je odluka projektanta i
...
Projektant dodaje dokumentaciju. Nalaznik povezuje dokaze, pronalazi primedbe, postavlja presudno pitanje, priprema povezane izmene i proverava novu reviziju nakon prihvatanja.
...
| Dokument | Namena |
|---|---|
| **[Nalaznik v3 — specifikacija i runbook](dokumenti/7-nalaznik-spec-v3.html)** | Aktuelni plan: proizvod, arhitektura, kompletan tok, setup, 18 zadataka, Git protokol i Claude promptovi |
| [Setup paket ZIP](dokumenti/nalaznik-v3-setup.zip) | 14 fajlova spremnih za kopiranje |
| [Setup direktorijum](dokumenti/nalaznik-v3-setup/) | Pojedinačni promptovi, skripte, šabloni i backlog |
...
| [Specifikacija v2](dokumenti/3-nalaznik-spec-v2.html) | Prethodni smer i originalni vizuelni format |
...
GitHub prikazuje HTML kao izvorni kod. Za pun prikaz klonirajte repo i otvorite `dokumenti/7-nalaznik-spec-v3.html` u browseru. Dokument ima sadržaj sa 30 odeljaka i dugmad za kopiranje promptova. Sačuvajte strukturu foldera da bi lokalne veze i preuzimanja radili.
...
2. Dogovorite ko je **A**, a ko **B**. U `dokumenti/nalaznik-v3-setup/team.example.json` vidite potrebna polja. Pri pripremi radnog `scripts/setup/team.json`, postavite `repo` na **`Excapex/GROKHakaton`** i unesite stvarne GitHub naloge za obe uloge. Ne ostavljajte `YOUR_OWNER/nalaznik` iz generičkog primera.
...
4. Zatim pratite kopiranje setup fajlova iz odeljka 21. U ovom klonu je putanja `$setupSource = (Resolve-Path '.\dokumenti\nalaznik-v3-setup').Path`. Zajednički `CLAUDE.md` ide u root, a izabrani lokalni dodatak u `CLAUDE.local.md`. Lokalna uloga i ključevi su ignorisani u Git-u.
...
**18 početnih zadataka, ukupno 43 SP.** Story poeni su relativna procena složenosti i rizika, ne sati i ne ocena doprinosa. Svaki issue ima jednog vlasnika, zavisnosti, kriterijum prihvatanja i dokaz završetka. Drugi član radi review. [Backlog sa kriterijumima](dokumenti/nalaznik-v3-setup/backlog.json).
...
|---|---|
| [CLAUDE.md](dokumenti/nalaznik-v3-setup/CLAUDE.md) | Zajednički protokol za oba člana; kopirati u root pri bootstrapu |
| [CLAUDE-A.local.md](dokumenti/nalaznik-v3-setup/CLAUDE-A.local.md) | Član A kopira kao `CLAUDE.local.md` |
| [CLAUDE-B.local.md](dokumenti/nalaznik-v3-setup/CLAUDE-B.local.md) | Član B kopira kao `CLAUDE.local.md` |
