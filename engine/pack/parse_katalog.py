...
IZLAZNI JSON sme u Git. .docx original NE SME — živi u lokalnom korpusu.
    ...
    export NALAZNIK_KATALOG="/put/do/Katalog_primedbi_ZOP_2026 ... .docx"
...
def main() -> int:
    raw = os.environ.get("NALAZNIK_KATALOG") or (sys.argv[1] if len(sys.argv) > 1 else "")
    if not raw:
        print("Postavi NALAZNIK_KATALOG na .docx katalog (lokalni korpus, ne Git).",
              file=sys.stderr)
        return 2
